...
// ADR-0025: the API reference tab is rendered from the bundled OpenAPI documents, one per service.
// The bundles are generated by scripts/build-openapi.mjs; only those, never the sources in src/, are
// published, because a source's shared components live in another file.
const OPENAPI = join(DOCS, '30-protocol', 'openapi');
const openapi = existsSync(OPENAPI)
  ? readdirSync(OPENAPI)
      .filter((f) => f.endsWith('.openapi.yaml'))
      .sort()
      .map((f) => `30-protocol/openapi/${f}`)
  : [];
...
const config = {
  ...
  navigation: {
    tabs: [
      ...
      ...(openapi.length ? [{ tab: 'API reference', openapi }] : []),
    ],
  },
  redirects,
};
