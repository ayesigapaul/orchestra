...
depends_on: [ADR-0001, ADR-0002, ADR-0005, ADR-0006, ADR-0007, ADR-0008, ADR-0009, ADR-0011]
---
...
version, not a version number copied onto the Run.
...
term. If the distinction survives review the glossary should be amended — a second word for the same
thing would be a defect.
...
of its own — a gap the glossary should close.
...
boundary. What persists is the **Policy Decision** — the rule that matched, the inputs, the verdict
and the timestamp — recorded for allows as well as denials.
...
- A Policy Decision references the Run, and where the enforcement point is a Step boundary, the Step
  Execution it gates.
- A Policy Decision names at most one Policy. Deny-by-default means the absence of a matching rule is
  itself a recorded outcome, so *no rule matched* is a decision, not a missing record.
...
- An Approval Chain requires decisions from one or more Principals, ordered or parallel, derived
  from policy rather than stored as a static list.
...
null. Audit is a product surface, not a log level.

The second diagram covers sections 5 to 7 together. A Tool whose origin is a native adapter has no
MCP Server edge, and per I1 only Tenant edges carrying meaning beyond tenant scoping are drawn.
...
erDiagram
  ...
  POLICY |o--o{ POLICY_DECISION : "matched by"
...
| Recorded governance outcome | Policy Decision | GLOSSARY |
...
the audit log; the glossary does not define *meter record*, and should.
...
| Open question | What would decide it |
| --- | --- |
| Attributes, keys, indexes, partitioning | The schema work that follows a datastore decision |
| The datastore engine | An architecture decision, constrained by ADR-0011 to one enforcing row-level security |
| Whether a Tool call in an Agent Run is a Step Execution, and whether a retry creates a new one | `execution-semantics.md` in [`../50-workflows/`](../50-workflows/), or an ADR |
| How platform operator action is attributed under I2 | The audit and threat models in [`../40-governance/`](../40-governance/) |
| Whether a cross-tenant person record exists, and how a Service Account authenticates | `identity-and-access.md` in [`../10-architecture/`](../10-architecture/) |
| Whether a Conversation may span Agents | A product decision, not yet taken |
| Whether Quota Envelopes are declared or discovered | The quota design ADR-0006 calls for |
| Audit and evidence retention periods | Compliance work; a customer contract will force it first |
| Entity state machines | `lifecycle-state-machines.md`, planned in [`./README.md`](README.md) |
