...
The Gateway resolves every credential it verifies, and fails closed.

### Changed

- `30-protocol/openapi/` — the Gateway's document (0.2.0). The identity probe answers with the
  Principal and Tenant a credential resolved to, and documents its 400, 415 and 503. The local
  security scheme requests the `organization` scope.
- `10-architecture/tech-stack.md` (0.33.0) — httpx2 is now a runtime dependency of the Gateway,
  which calls Tenant User Management with it.
...
The published documentation site is linked from the repository, and links its author.

### Changed

- `README.md` (0.33.0) — section 3 names the published site's address, and notes that `docs.json`
  links the author's site from the navbar and footer.
- `docs.json`, generated by `scripts/build-docs-nav.mjs` — an Author link in the navbar, and the
  author's site and the repository in the footer.
...
The datastore has its first schema, and the migration tool is chosen.

### Changed

- `10-architecture/tech-stack.md` (0.29.0) — section 3 chooses dbmate for migrations: plain SQL,
  strictly ordered, and applied to each service's own schema as its owner role, with provisioning
  creating the roles. golang-migrate, Atlas, Sqitch and Flyway were weighed. Section 1.1 pins dbmate
  2.35.1.
