...
| --- | --- | --- |
| **Gateway** | The plane's only ingress. Authenticates the calling Principal by any credential the Gateway contract accepts, resolving it to exactly one Principal and one Tenant; forwards a Run submission to the Run Supervisor, which admits it; terminates the Run event stream. It keeps no durable state and evaluates no Policy. | `gateway-api.md` and `event-protocol.md` in [`../30-protocol/`](../30-protocol/); [ADR-0034](../adr/adr-0034-policy-decisions-commit-with-the-gated-change-and-leave-by-outbox.md) |
...
| **Run Supervisor** | Run lifecycle, worker leasing, per-Tenant concurrency, scheduling and wake-up — and the enforcing service at admission: it evaluates admission in process and commits the Run, or its refusal, with the admission Policy Decision in one transaction of its own schema, which then leaves through its own outbox. | [ADR-0014](../adr/adr-0014-run-supervisor-is-orchestras.md), [ADR-0019](../adr/adr-0019-postgres-run-supervisor.md), [ADR-0034](../adr/adr-0034-policy-decisions-commit-with-the-gated-change-and-leave-by-outbox.md) |
| **Policy Enforcement Points** | Not a component — places in the path where policy is evaluated and enforced. The evaluator is not a component either: it runs in process in each enforcing service; section 5. | [`policy-model.md`](../40-governance/policy-model.md) E1; [ADR-0035](../adr/adr-0035-cel-profile-for-policies-and-workflow-expressions.md) |
| **Durable decision write** | The enforcing service's own transaction, which commits the Policy Decision with the state change it gates before the gated action is attempted; the decision then reaches the audit store through that service's outbox. It is drawn below as a participant because the write is on the path. | [ADR-0013](../adr/adr-0013-fail-closed-policy-decision-writes.md), [ADR-0034](../adr/adr-0034-policy-decisions-commit-with-the-gated-change-and-leave-by-outbox.md) |
...
sequenceDiagram
  ...
  G->>S: Forward the submission, Principal and Tenant already resolved
  S->>P: Admission enforcement point, evaluated in process
  P->>D: Write the Policy Decision, durably. It must survive a crash
  P-->>S: allow, deny or require_approval
  S-->>G: The Run as created or refused, with its identifier
  G-->>C: A deny ends it here. The Run is Denied and no Step ever runs
  S->>R: Admitted. Definition and Policy versions pinned for life
  ...
  D-->>D: Delivered to the audit store afterwards, through the enforcing service's outbox
```

| Control | Where it sits | What it gates | Normative home |
| --- | --- | --- | --- |
| Admission enforcement point | Run Supervisor, before the Run reaches the Runtime; the Gateway forwards the submission and evaluates nothing | Whether the Run exists at all | [`policy-model.md`](../40-governance/policy-model.md) E1; [`lifecycle-state-machines.md`](../20-domain/lifecycle-state-machines.md) 2.1; [ADR-0034](../adr/adr-0034-policy-decisions-commit-with-the-gated-change-and-leave-by-outbox.md) |
...
Gateway forwards the submission and evaluates nothing; the **Run Supervisor** is the enforcing
service, committing the Run, or its refusal, with the admission Policy Decision in one transaction
of its own schema
([ADR-0034](../adr/adr-0034-policy-decisions-commit-with-the-gated-change-and-leave-by-outbox.md)).
That makes the supervisor a governance component and leaves the Gateway a boundary that keeps no
...
[`system-context.md`](system-context.md) both assign this question here, and it is settled:
evaluation runs in process at each enforcing service, over the Policy versions the Run pinned.
[ADR-0035](../adr/adr-0035-cel-profile-for-policies-and-workflow-expressions.md) records it among
its consequences. It needed no ADR of its own, because no evaluation needs datastore access of its
own.

Three decided things point there, and the enforcement path is where they bite.
...
the Run pinned, with distribution of those versions a Control Plane read. Its premise holds: an
in-process evaluator holds no cross-Run total, and a Policy is a pure function of its supplied
inputs, because ADR-0035 lets a Policy read only platform-defined aggregates, each read and reserved
in the transaction that writes its decision
([ADR-0034](../adr/adr-0034-policy-decisions-commit-with-the-gated-change-and-leave-by-outbox.md)).
[`containers.md`](containers.md) section 5 records the same answer.

Two things follow, and both are this plane's. Evaluation failure is a fail-closed failure rather
than a degraded one (ADR-0013). And the in-process shape carries a cost that is accepted rather than
avoided: an evaluator shipped inside every enforcing service makes a fix a fleet rollout rather than
the single upgrade a separate component would take, so a partially rolled-out fleet is a correctness
problem, which the Expression Profile's conformance cases, run by every enforcing service, exist to
catch (ADR-0035).
...
Records may degrade if the degradation is recoverable from the trail.
[ADR-0034](../adr/adr-0034-policy-decisions-commit-with-the-gated-change-and-leave-by-outbox.md)
fixes the mechanism: the enforcing service's own transaction, then that service's outbox. The rules
are not restated here; their consequences for this plane's shape are.

**The plane is stateless.** A Policy Decision commits in the enforcing service's own transaction,
with the state change it gates, so no process owns durable storage, and scheduling, restart, drain
and replacement need not account for any. What the write puts on the enforcement path is a commit
to the datastore the enforcing service writes to, which the gated state change needs anyway, and
that datastore's availability is the ceiling ADR-0013 accepts knowingly. No store outside the
row-level-secured datastore is introduced: a decision waits in the enforcing service's schema under
forced row-level security, its outbox included, and leaves through the Kafka topics
[`multi-tenancy.md`](multi-tenancy.md) section 8 already registers.

**Delivery lag bounds how current an audit query can be.** A decision reaches the audit store after
its transaction commits, through the outbox, Debezium and Kafka
([ADR-0029](../adr/adr-0029-kafka-carries-facts-captured-by-debezium.md)), and the audit store is
read through the Control Plane, so a decision that has already gated an action is durable and not
yet readable. Anyone reading a Run in flight — approver, auditor, support engineer — reads a
trailing view, and the audit surface has to say what it has not yet seen: absence read as
non-occurrence is the failure ADR-0013 exists to prevent, arriving by another route. ADR-0013 makes
the lag a governed property and gives the signal, the completeness horizon, to `observability.md`
in [`../60-operations/`](../60-operations/). A stalled capture delays audit rather than losing it,
because ADR-0034 keeps each decision in the enforcing service's schema until the audit store has
recorded it, and a decision cannot be regenerated
([`policy-model.md`](../40-governance/policy-model.md) D5).

**Enforcement-point count is a latency multiplier.** A durable commit sits ahead of every gated
action, so a Workflow with many Steps pays it at every boundary.
...
outside the datastore; section 6 answers for this plane that the durable decision write introduces
none, its decisions leaving through the Kafka topics that document already registers.
