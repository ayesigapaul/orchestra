...
| The shape and scope of the egress allow-list | Section 9's observation that two lists exist facing opposite directions with different owners, which [ADR-0038](../adr/adr-0038-egress-proxy-on-a-per-tenant-allow-list.md) keeps: it decides Orchestra's outbound list for the Model Broker and Tool Invocation and leaves this document's share untouched | Escalated — the connector's share alone |
...
| The Connector's share of the egress allow-list; the default-deny posture is settled normatively by [`../40-governance/threat-model.md`](../40-governance/threat-model.md) T6, and the shape for the Model Broker and Tool Invocation by [ADR-0038](../adr/adr-0038-egress-proxy-on-a-per-tenant-allow-list.md) | This document with ADR-0038, after ADR-0007 binds; the connector's share cannot close alone while ADR-0007 is **Proposed** | **ADR required** — it extends ADR-0038 to a fourth outbound edge; *repeated* from [`containers.md`](containers.md) and [`data-plane.md`](data-plane.md) |
