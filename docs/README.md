...
Orchestra is a **multi-tenant SaaS governance and connectivity layer for enterprise AI agents.** It is not an agent framework. The orchestration runtime (LangGraph), the model providers, and the tool protocol (MCP) are treated as commodity rails. Orchestra supplies the control surface over them: identity, policy, approval, audit, connectivity, workflow definition, and metering.

> **Product thesis.** Deterministic where determinism matters. Agentic where judgment matters. Governed at every step.
...
Directory prefixes (`00-`, `10-`, …) fix reading order and leave room for insertion. They are part of the path and must not be renumbered once published — see [VERSIONING.md](VERSIONING.md).
...
**File naming.** Lowercase kebab-case, `.md`. ADRs use `adr-NNNN-short-title.md`. RFCs use `rfc-NNNN-short-title.md`. Schemas use `<entity>.v<MAJOR>.schema.json`. No spaces, no dates in filenames except in `archive/`.

**Diagrams.** Mermaid, inline in the Markdown, so diagrams version with the prose and diff in review. Every inline diagram is also rendered to a browsable HTML page in `assets/diagrams/` — open [`assets/diagrams/index.html`](assets/diagrams/index.html) in a browser. Those pages are generated by `scripts/build-diagrams.mjs` and never edited by hand, and CI fails when one no longer matches its source, so re-run the script after changing a diagram. Only when Mermaid cannot express a diagram (dense sequence charts, physical topology) may a hand-made SVG live beside them — and it must ship with its source.

**Published site.** The set is published with [Mintlify](https://mintlify.com), configured by `docs.json` in this directory. That file is generated by `scripts/build-docs-nav.mjs` and not edited by hand: it takes the navigation from the directory tree and each document's `title`, and adds a redirect per page so the `../section/page.md` links that GitHub follows also resolve on the site.

**Architecture views** follow the [C4 model](https://c4model.com): Level 1 system context, Level 2 containers, Level 3 components where warranted. No Level 4.

**Requirement keywords** — MUST, MUST NOT, SHOULD, SHOULD NOT, MAY — carry their [RFC 2119](https://www.rfc-editor.org/rfc/rfc2119) meanings and are written in capitals.

**Normative vs informative.** Sections in `30-protocol/` and `40-governance/` are normative: implementations must conform. Everything else is informative unless it says otherwise.

**Decisions live in ADRs, not in prose.** If a document explains _why_ an approach was chosen, it links to an ADR. If no ADR exists, the decision is not yet made — say so explicitly rather than implying consensus.

---

## 4. Status of this documentation set

This set is at **v0.19.0** and is **pre-implementation**. It supersedes the single-file v0.1 vision brief, retained at [`archive/vision-v0.1-2026-09-08.md`](archive/vision-v0.1-2026-09-08.md).
...
Decisions taken since — tenant isolation, audit semantics, the run supervisor and the positioning that supersedes ADR-0003 — are in the [ADR index](adr/README.md), which is the complete list.
...
2. Once a decision is reached, it is recorded as an **ADR**. ADRs are immutable once Accepted — they are superseded, never edited.
3. Documents affected by the decision are then updated, and [`CHANGELOG.md`](CHANGELOG.md) records the change against the documentation-set version.
4. Protocol and schema changes additionally follow the compatibility rules in [VERSIONING.md](VERSIONING.md) and require a contract test before merge.
