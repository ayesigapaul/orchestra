...
**Published site.** The set is published with [Mintlify](https://mintlify.com) at
[orchestra-28364c7e.mintlify.site](https://orchestra-28364c7e.mintlify.site/section-index),
configured by `docs.json` in this directory. That file is generated by `scripts/build-docs-nav.mjs`
and not edited by hand: it takes the navigation from the directory tree and each document's `title`,
adds a redirect per page so the `../section/page.md` links that GitHub follows also resolve on the
site, and links the author's site from the navbar and footer.
...
This set is at **v0.36.0**, and implementation has begun — **Phase 0**, the foundation. It
